...
namespace UMW\Advisories {
	if ( ! class_exists( 'Plugin' ) ) {
		class Plugin {
			 ...
			 * @access public
			 */
			public static string $version = '2023.06.26.01';
			...
			public function do_init() {
				...
				add_filter( 'rest_endpoints', array( $this, 'rest_order_meta' ), 10, 2 );
			...
			public function register_meta_fields() {
				$tmp = register_meta( 'post', '_advisory_expires_time', array(
					'type'         => 'string',
					'description'  => __( 'The time the advisory should expire' ),
					'single'       => true,
					'show_in_rest' => true
				) );
				...
				$tmp = register_meta( 'post', '_advisory_permalink', array(
					'type'              => 'string',
					'description'       => __( 'The original location of this advisory' ),
					'single'            => true,
					'sanitize_callback' => 'esc_url',
					'show_in_rest'      => true
				) );
				Debug::log( '[Alerts API debug]: Registered the permalink meta field with a result of ' . print_r( $tmp, true ) );
				$tmp = register_meta( 'post', '_advisory_author', array(
					'type'         => 'string',
					'description'  => __( 'The name of the advisory author' ),
					'single'       => true,
					'show_in_rest' => true
				) );
			 ...
			 *
			 * @param array $vars the existing array of arguments
