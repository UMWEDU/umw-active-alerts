class UmwActiveAlerts {
    ...
    log(message) {
        ...
        if ( 'alerts_url' in this ) {
            // Avoid logging on production
            if ( this.alerts_url.indexOf('umw.edu') >= 0 ) {
                return;
            }
        }
    ...
    async init() {
        ...
        await this.doGlobalEmergency().then(async () => {
    ...
    getQueryDate() {
        // Attempt to avoid using too many server resources by only busting the cache once/minute, rather than potentially every millisecond
        const ms = 60 * 1000; // Milliseconds in a minute
        const roundedTime = new Date(Math.round(this.now.getTime() / ms) * ms);
    ...
    handleFetchError(error) {
        if (typeof error.json === "function") {
            error.json().then(jsonError => {
                ...
                this.log(genericError.statusText);
            });
        } else {
    ...
    insertGlobalAlert(e) {
        ...
        if ( document.querySelectorAll('.emergency-alert').length > 0 ) {
            const emergency = document.querySelector('.emergency-alert');
    ...
    async doGlobalEmergency() {
        await this.doAlert('emergency');
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    const alert = await new UmwActiveAlerts();
});